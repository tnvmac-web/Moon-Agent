...
class OpenAICompatibleLLM(LocalLLM):
    ...
    def __init__(self, base_url: str = "http://localhost:1234/v1", api_key: str = "not-needed", model: str = "local-model"):
        self.base_url = base_url.rstrip('/')
    ...
    def chat(self, messages: List[Dict[str, str]], **kwargs) -> str:
        ...
        response = requests.post(f"{self.base_url}/chat/completions", json=payload, headers=self.headers, timeout=120)
        response.raise_for_status()
...
class NVIDIANIM_LLM(LocalLLM):
    ...
    def __init__(self, model: str = "meta/llama-3.1-8b-instruct", api_key: str = None, base_url: str = "https://integrate.api.nvidia.com/v1"):
        self.model = model
