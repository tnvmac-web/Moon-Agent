...
def test_skills():
    ...
    for skill in skills:
        manager._tool_registry.update(
            {
                name: {"skill_name": "builtin", "tool_info": info}
                for name, info in skill.get_tools().items()
            }
        )
...
def test_commands():
    ...
    from commands.commands import create_command_registry
    from config.settings import get_settings
    ...
    class MockApp:
        def __init__(self):
            ...
            self.dark = False
            from memory.memory import create_memory_store
            from models.local_llm import create_llm
            from skills.skills import SkillManager, create_builtin_skills
