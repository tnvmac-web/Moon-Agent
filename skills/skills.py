...
class SkillManager:
    ...
    def discover_skills(self) -> List[Path]:
        ...
        for item in self.skills_dir.iterdir():
            if item.is_dir() and (item / "skill.yaml").exists():
                skills.append(item)
            elif item.is_dir() and (item / "skill.json").exists():
                skills.append(item)
    ...
    def load_skill(self, skill_path: Path) -> Optional[Skill]:
        ...
        try:
            ...
            if not skill_class:
                for name, obj in inspect.getmembers(module):
                    if inspect.isfunction(obj) and name in ('create_skill', 'skill_main', 'get_skill'):
                        skill_factory = obj
                        break
            ...
            for tool_name, tool_info in skill.get_tools().items():
                if tool_name in self._tool_registry:
                    logger.warning(f"Tool name conflict: {tool_name} (from {self._tool_registry[tool_name]['skill_name']} and {manifest.name})")
...
class CodeExecutionSkill(Skill):
    ...
    def initialize(self) -> bool:
        self.register_tool(
            ...
            {"code": {"type": "string", "description": "Python code to execute"}, "timeout": {"type": "integer", "default": 30}}
        )
        self.register_tool(
            "run_shell",
            self.run_shell,
            "Execute shell command",
            {"command": {"type": "string", "description": "Shell command"}, "timeout": {"type": "integer", "default": 30}}
        )
        return True
...
class FileOperationsSkill(Skill):
    ...
    def initialize(self) -> bool:
        self.register_tool("read_file", self.read_file, "Read file contents", 
                          {"path": {"type": "string", "description": "File path"}})
        self.register_tool("write_file", self.write_file, "Write content to file",
                          {"path": {"type": "string", "description": "File path"}, "content": {"type": "string", "description": "Content to write"}})
        self.register_tool("list_files", self.list_files, "List directory contents",
...
class WebSearchSkill(Skill):
    ...
    def initialize(self) -> bool:
        self.register_tool("web_search", self.web_search, "Search the web",
                          {"query": {"type": "string", "description": "Search query"}, "max_results": {"type": "integer", "default": 5}})
        self.register_tool("fetch_url", self.fetch_url, "Fetch and extract text from URL",
                          {"url": {"type": "string", "description": "URL to fetch"}, "max_chars": {"type": "integer", "default": 5000}})
        return True
