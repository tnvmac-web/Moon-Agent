...
class MemoryStore:
    ...
    def __init__(self, db_path: str = "memory.db", vector_store: Optional[VectorStore] = None):
    ...
    async def add_async(self, entry: MemoryEntry) -> str:
        ...
        # Add to vector store
        self.vector_store.add(entry.id, entry.embedding, {
            'type': entry.type,
    ...
    def update(self, entry: MemoryEntry) -> bool:
        ...
        """, (
            entry.content, json.dumps(entry.metadata), entry.updated_at,
            entry.importance, json.dumps(entry.tags), entry.id
        ))
        conn.commit()
        conn.close()
