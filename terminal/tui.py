...
from textual.widgets import (
    Header, Footer, Input, RichLog, Button, Select, 
    Static, TabbedContent, TabPane, DataTable, Label
)
...
from commands.commands import create_command_registry, CommandContext
...
class TUIApp(App):
    ...
    def __init__(self):
        ...
        self.skill_manager = SkillManager(
            ...
            config=self.settings.settings.skills.model_dump() if hasattr(self.settings.settings.skills, 'model_dump') else {}
        )
    ...
    def action_switch_agent(self) -> None:
        ...
        current_idx = agents.index(self.current_agent_type) if self.current_agent_type in agents else 0
    ...
    async def refresh_sidebar_data(self):
        ...
        if self.skill_manager:
            ...
            if skills:
                content = []
                for skill in skills:
                    content.append(f"[bold]{skill.name}[/bold] v{skill.version}\n[dim]{skill.description}[/dim]")
                skills_list.update("\n\n".join(content))
            else:
    ...
    def refresh_status(self):
        ...
        status = [
            ...
            f"Session: [bold]{self.current_session_id[:20]}...[/bold]" if self.current_session_id else "No session",
